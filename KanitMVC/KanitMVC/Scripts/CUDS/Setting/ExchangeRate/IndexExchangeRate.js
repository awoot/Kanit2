...
$(document).ready(function () {
    ...
    $("#chkHistory").on("click", function () {
        //this.checked
        var items = vm.get("dataSource");

        for (var i = 0; i < items.length; i++) {
            items[i].IsDisplay = items[i].IsHistory == 0 || this.checked;
        }

        vm.set("dataSource", []);
        vm.set("dataSource", items);


        $("#tblExchangeRate tr[data-temp]").remove();
    });
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.Table.length; i++) {
                    ...
                    data.Table[i].IsDisplay = data.Table[i].IsHistory == 0;
                }
